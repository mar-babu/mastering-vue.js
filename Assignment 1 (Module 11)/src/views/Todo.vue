...
<template>
    <div class="min-h-screen w-screen bg-gradient-to-r from-blue-300 to-sky-300 flex flex-col gap-2 p-10 items-center">
        <div class="max-w-2xl w-full bg-white p-2 rounded-sm shadow-lg">
            <form enctype="multipart/form-data" class="grid grid-cols-12 gap-2">
                ...
                <div class="col-span-3 flex items-center">
                    <input @click.prevent="add()" class="w-full px-5 py-2 bg-gray-400 text-white rounded-md font-semibold hover:bg-sky-500  hover:cursor-pointer" type="submit" name="submit" value="Add new Task">
                </div>
            </form>
        </div>
                    ...
                    <tr v-for="(pending, index) in todo.pending" :key="index">
                        ...
                        <td class="p-2 border border-slate-300 flex gap-1 justify-end">
                            <form enctype="multipart/form-data">
                                <input @click.prevent="todo.action.update(pending.id, doneFormData)" type="submit" name="submit" value="Mark as Done" class="px-2 bg-gray-400 text-white rounded-md hover:bg-sky-600 font-semibold hover:cursor-pointer">
                            </form>
